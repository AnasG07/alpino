...
function OAuthError() {
  ...
  const { error_description: errorDescription, error_hint: errorHint } = queryString.parse(location.search);

  return (
    <div>
      <div className={classes.pageTitle}>{t('error')}</div>
      <div className={classes.generalText}>{t('authError')}</div>
