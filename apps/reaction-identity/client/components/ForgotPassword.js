...
function ForgotPassword() {
  ...
  const [sendResetAccountPasswordEmail, { error: mutationError, loading: isMutationLoading }] = useMutation(
    sendResetAccountPasswordEmailMutation,
    {
      onCompleted() {
  ...
  return (
    <div>
      ...
      {submitWasSuccessful && (
        <InlineAlert alertType="success" className={classes.inlineAlert} message={t('passwordResetSend')} />
      )}
      <Button actionType="important" isFullWidth isWaiting={isMutationLoading} onClick={submitForm}>
        {t('resetYourPassword')}
      </Button>
      <Button
