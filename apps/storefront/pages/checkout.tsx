...
import useAddAccountAddressBookEntry from '../hooks/address/useAddAccountAddressBookEntry.js';
...
function Checkout({ items }) {
  ...
  const [renderCheck, updateRenderCheck] = useState(false);
  const [saveLater, updateSaveLater] = useState(false);
  ...
  useEffect(() => {
    ...
    if (cart.checkout && !isEmpty(cart?.checkout)) {
      updateAddress({
      ...
      updateEmail(cart?.email);
    }
  }, [cart?.email, cart?.checkout?.fulfillmentGroups[0]?.shippingAddress]);

  const onSubmit = async (e) => {
    e.preventDefault();

    setLoading(true);
    await onSetShippingAddress(address);
    await setEmailOnAnonymousCart({ email });
    //  if (saveLater) {
    //  await addAccountAddressBookEntry(address);
    //}
    setLoading(false);
      ...
      <main>
        <div className={classNames(style.backgroundColor, 'min-h-screen bg- w-full pt-24')}>
          ...
          <div>
            <form
              onSubmit={onSubmit}
              className="mt-10 min-h-screen flex flex-col items-center px-6 pb-24 lg:flex-row justify-around">
              <div className={classNames(style.card, 'px-8 mb-8 lg:mb-0 mx-6 lg:mx-0')}>
                ...
                <div className="flex items-baseline">
                  <input type="checkbox" onChange={() => updateSaveLater(!saveLater)} checked={saveLater} />
                ...
                <div className="flex flex-row w-full justify-center items-center mt-4">
                  <button type="button" onClick={() => Router.back()} className="outline-none border-none mb-8">
              ...
              <div
                className={classNames(
                  style.card1,
                  'px-8 flex flex-col items-center justify-around lg:justify-between',
                )}>
                    ...
                    <div className={classNames(style.itemCards, 'flex flex-row mb-8 w-full')}>
                      ...
                      <div className="flex flex-col w-full justify-between items-start">
                        <div className="flex justify-between px-4 w-full">
                          <Link href={`products/${item?.productSlug}`}>
                            <a>
                              <h1 className={classNames(style.titleFontColor, 'text-2xl font-medium')}>
                                {item?.title}
                              </h1>
                            </a>
                          </Link>
                ...
                <div className={classNames(style.smallCard, 'px-6')}>
                  ...
                  <div className="flex flex-row items-center justify-between mt-6 px-4">
                    <h1 className={classNames(style.greenFontColor, 'text-lg font-normal tracking-wider ml-4')}>
                      Total
                    </h1>
